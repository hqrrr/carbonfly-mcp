...
See [Carbonfly installation guide](https://github.com/RWTH-E3D/carbonfly/blob/master/HowToInstall.md) for WSL and OpenFOAM setup.
...
```
Install and configure the Carbonfly MCP server.
Read the instructions at:
https://raw.githubusercontent.com/RWTH-E3D/carbonfly-mcp/refs/heads/main/mcp/README.md
...
> **Note**: When using OhMyOpenCode, include `ulw` in your prompt for best results.
...
`<python-path>` is the path to your Python executable (e.g. `C:\\Users\\...\\Python310\\python.exe`).
`<path-to-mcp>` is the absolute path to the `carbonfly-mcp\\mcp` directory.
...
<details>
<summary><b>Codex</b> — add to config TOML (<code>~/.codex/config.toml</code>)</summary>
...
Carbonfly MCP organizes everything under a **workshop directory** (created by `create_workshop`). Understanding where files live helps you inspect, debug, and bring your own geometry.
...
└── cases/
    └── <case_name>/                  # OpenFOAM case directory
        ...
        │   ├── fvModels, g, ...      # OpenFOAM constant dictionaries
...
Place your existing STL files directly into the workshop's `stl/` folder, **then** call `import_stl` to register them:
...
You can also inspect the STL files directly with any STL viewer (e.g., Blender, FreeCAD, MeshLab).

### Where to find generated cases

After calling `generate_case`, the case is at:

```
<workshop>/cases/<case_name>/
```

Use `get_workshop_info` to see the full path:

```
@carbonfly get_workshop_info for my workshop
```

### Where to find simulation results

Results (field snapshots) appear as numbered time directories inside the case:

```
<workshop>/cases/<case_name>/1/      # CO2, U, T at t=1s
<workshop>/cases/<case_name>/10/     # at t=10s
<workshop>/cases/<case_name>/60/     # at t=60s (e.g., final time)
```

Use `list_results` to see which time directories exist and what fields they contain.

### How to view 3D results (ParaView)

**ParaView must be installed separately** — download from [paraview.org](https://www.paraview.org/download/).

1. Open ParaView
2. **File → Open** → navigate to `<workshop>/cases/<case_name>/`
3. Select the `<case_name>.foam` file
4. In the **Properties** panel, click **Apply**
5. Select fields to visualize from the dropdown: `CO2`, `U` (velocity), `T` (temperature), `p` (pressure)
6. Use **Filters** (e.g., Slice, Contour, Glyph) for advanced visualization
...
Use the post-processing tools in the MCP:
...
All case files are plain text (OpenFOAM dictionary format). You can open them in any text editor:

- **Mesh settings**: `system/blockMeshDict`, `system/snappyHexMeshDict`
- **Solver settings**: `system/controlDict`, `system/fvSchemes`, `system/fvSolution`
- **Initial conditions**: `0/U`, `0/T`, `0/CO2`, ...
- **Boundary config**: `boundaries.json` (in workshop root)

### How to re-run or modify a simulation

1. Edit case files manually or re-run `generate_case` (overwrites all files)
2. Clear previous results: delete time directories (keep only `0/`, `constant/`, `system/`)
3. `run_meshing` then `run_simulation` again

Or:
1. Create a new case with different parameters: `generate_case` with a new `case_name`
2. This keeps the previous case intact for comparison
...
```
mcp/
├── .agents/skills/carbonfly-mcp-use/   # Agent skill (usage guide)
│   └── SKILL.md
├── src/carbonfly_mcp/                  # MCP server source
...
LGPL-3.0 – see the [LICENSE](../LICENSE) file.
